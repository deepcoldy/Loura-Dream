...
(function() {
  ...
  CanvasInput.prototype = {
    ...
    selectText: function(range) {
      var self = this,
        _range = range || [0, self._value.length];

      // select the _range of text specified (or all if none specified)
      setTimeout(function() {
        self._selection = [_range[0], _range[1]];
        self._hiddenInput.selectionStart = _range[0];
        self._hiddenInput.selectionEnd = _range[1];
        self.render();
      }, 1);
    ...
    _overInput: function(x, y) {
      var self = this,
        ...
        yBottom =
          y <= self._y + self._extraY + self._height + self._padding * 2;
